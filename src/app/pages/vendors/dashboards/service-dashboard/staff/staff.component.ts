              ...
              <div>
                ...
                <div class="flex items-center gap-4">
                  @if (currentForm.profileImage) {
                    <img [src]="currentForm.profileImage" alt="Profile" class="w-24 h-24 rounded-lg object-cover border-2 border-blue-300">
                  } @else {
...
export class ServiceStaffComponent implements OnInit {
  ...
  onProfileImageSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      const uploadPath = `staff/${this.providerId}`;
      this.uploadService.uploadImage(file, uploadPath).subscribe({
        next: (url: string) => {
          this.currentForm.profileImage = url;
