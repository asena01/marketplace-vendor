...
export interface Product {
  ...
  // Variant support (for all product types)
  size?: string[];   // Available sizes
  color?: string[];  // Available colors (general, not just furniture)
...
export class ProductService {
  ...
  private getVendorHeaders(): HttpHeaders {
    // Use userId as vendorId - it's the unique identifier for the vendor/user
    const userId = localStorage.getItem('userId');

    return new HttpHeaders({
      'Content-Type': 'application/json',
      'x-user-id': userId || '',
      'x-vendor-id': userId || ''  // Use userId as the vendor ID - it's the unique identifier
    });
  }
  ...
  getVendorInventory(vendorId: string): Observable<any> {
    const headers = this.getVendorHeaders();
    return this.http.get<any>(`${this.apiUrl}/vendor/${vendorId}/inventory`, { headers });
  }

  // Get low stock products
  getLowStockProducts(vendorId: string, threshold: number = 5): Observable<any> {
    const headers = this.getVendorHeaders();
    return this.http.get<any>(`${this.apiUrl}/vendor/${vendorId}/low-stock?threshold=${threshold}`, { headers });
  }

  // Update product stock
  updateProductStock(productId: string, quantity: number, action: 'set' | 'add' | 'subtract' = 'set'): Observable<ApiResponse<any>> {
    const headers = this.getVendorHeaders();
    return this.http.put<ApiResponse<any>>(`${this.apiUrl}/${productId}/stock`, { quantity, action }, { headers });
  }
}
